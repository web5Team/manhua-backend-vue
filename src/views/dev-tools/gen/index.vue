    ...
    <template #wrapper>
      <el-card class="box-card">
        ...
        <el-table v-loading="loading" border :data="tableList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="菜单名称" align="center" prop="tableComment" :show-overflow-tooltip="true" width="130" />
      ...
      <import-table ref="importTB" @ok="getList" />
    </template>
  </BasicLayout>
</template>

<script>
import { listTable, previewTable, delTable, toDBTable, toProjectTableCheckRole, apiToFile } from '@/api/dev-tools-gen'
...
export default {
  ...
  methods: {
    ...
    handleGenTable(row) {
      ...
      downLoadFile('/admin/v1/gen/gencode/' + ids)
    },
    /** 打开导入表弹窗 */
    openImportTable() {
    ...
    handlePreview(row) {
      previewTable(row.tableId).then(response => {
        ...
        this.codeChange('template/controller.go.template')
      })
    },
    handleToProject(row) {
    ...
    handleEditTable(row) {
      ...
      this.$router.push({ path: '/dev-tools/gen/editTable', query: { tableId: tableId }})
    },
    /** 删除按钮操作 */
    handleDelete(row) {
    ...
    handleSingleDelete(row) {
      const tableIds = (row.tableId && [row.tableId]) || this.ids
      delTable({ ids: tableIds }).then((response) => {
        if (response.code === 200) {
          this.msgSuccess(response.msg)
          this.open = false
          this.getList()
