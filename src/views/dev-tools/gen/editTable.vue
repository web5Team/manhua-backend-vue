    ...
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane label="字段信息" name="cloum">
        ...
        <el-table :data="columns" :max-height="tableHeight" style="width: 100%">
          ...
          <el-table-column label="排序" width="70" :render-header="renderHeadeSort" :cell-style="{'text-align':'center'}">
            <template slot-scope="scope">
...
<script>
import { getGenTable, updateGenTable, getTableTree } from '@/api/dev-tools-gen'
...
export default {
  ...
  methods: {
    ...
    renderHeadeSearch(h, { column, $index }) {
      return h('div', [
        ...
        h(
          ...
          { props: { placement: 'top-start', width: '270', trigger: 'hover' }},
          [
            h('p', '是否当做搜索条件，打√表示做为搜索条件', { class: 'text-align: center; margin: 0' }),
            h('i', { class: 'el-icon-question', style: 'color:#ccc,padding-top:5px', slot: 'reference' })
          ]
        )
      ])
    },
    renderHeadeSort(h, { column, $index }) {
      return h('div', [
        h('span', column.label + '  ', { align: 'center', marginTop: '0px' }),
        h(
          'el-popover',
          { props: { placement: 'top-start', width: '270', trigger: 'hover' }},
          [
            h('p', '是否支持排序，打√表示支持排序', { class: 'text-align: center; margin: 0' }),
            h('i', { class: 'el-icon-question', style: 'color:#ccc,padding-top:5px', slot: 'reference' })
          ]
        )
      ])
    ...
    close() {
      ...
      this.$router.push({ path: '/dev-tools/gen/index', query: { t: Date.now() }})
    }
  }
}
</script>
