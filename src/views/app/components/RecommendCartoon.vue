<template>
  <div class="root">
    <el-form ref="formData" :inline="true" :model="formData" size="small" class="demo-form-inline">
      ...
      <el-form-item>
        <el-button v-if="editForm.templateId === -1" type="primary" size="mini" icon="el-icon-plus" v-permisaction="['admin:appReco:add']" @click="add">新增</el-button>
        <el-button v-if="editForm.templateId === -1" type="danger" size="mini" icon="el-icon-delete" v-permisaction="['admin:appReco:remove']" :disabled="ids.length == 0" @click="deleteData">删除</el-button>
      </el-form-item>
    </el-form>
    <el-divider />
    ...
    <el-dialog title="当前栏目" :visible.sync="openChapter" :append-to-body="true" width="60%" :close-on-click-modal="false">
      <template slot="title"
        >当前偏好: {{ formData.gender | arrFilter(['value', 'label'], preferencesOptions) }}</template
      >
...
import { recommendCartoonListFeach, addRecommendCartoonFeach, delRecommendCartoonFeach } from '@/api/app'
import { listTemRecommend } from '@/api/moduleSet'
...
export default {
  ...
  data() {
    return {
      ...
      preferencesOptions: [{ value: 1, label: '男生' }, { value: 2, label: '女生' }, { value: 3, label: '跳过' }],
      recommendQueryEd: false //判断书架有没有请求过
    }
  },
  computed: {
    isButtom() {
  ...
  watch: {
    ...
    tapType(val) {
      if (val === 'six' && !this.recommendQueryEd) {
        this.recommendQueryEd = true
        this.creatData()
  ...
  methods: {
    ...
    async deleteList() {
      await delRecommendCartoonFeach({ appId: this.formData.appId, ids: this.ids })
      this.getList()
    ...
    async getList() {
      ...
      if (this.editForm.templateId !== -1) {
        api = listTemRecommend
        delete paramsObj.appId
      } else {
        api = recommendCartoonListFeach
    ...
    async addBook() {
      ...
      const { code } = await addRecommendCartoonFeach(form)
      if (code === 200) {
