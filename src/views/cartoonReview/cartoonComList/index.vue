    ...
    <template #wrapper>
      <el-card class="box-card">
        <el-form ref="queryForm" :model="queryParams" :inline="true" label-width="68px">
          ...
          <el-form-item label="漫画名称" prop="cartoonName">
            <el-input v-model="queryParams.cartoonName" placeholder="请输入漫画名称" clearable size="small" @keyup.enter.native="handleQuery" />
          </el-form-item>
          <el-form-item label="评论人id" prop="memberId">
            <el-input v-model="queryParams.memberId" placeholder="请输入评论人id" clearable size="small" @keyup.enter.native="handleQuery" />
          </el-form-item>
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              v-permisaction="['admin:cartoonComList:remove']"
              type="danger"
              icon="el-icon-delete"
              size="mini"
              :disabled="multiple"
          ...
          <el-col :span="1.5">
            <el-dropdown v-permisaction="['admin:cartoonComList:change']" style="margin-left: 10px;" @command="changeStatues">
        ...
        >
          ...
          <el-table-column label="漫画id" align="center" prop="cartoonId" width="110" :show-overflow-tooltip="true" sortable="custom" />
          <el-table-column label="漫画名称" align="center" prop="cartoonName" width="200" :show-overflow-tooltip="true" />
          <el-table-column label="评论人id" align="center" prop="memberId" width="110" :show-overflow-tooltip="true" sortable="custom" />
...
<script>
import { deleteCartoonComment, listCartoonComment, editCartoonCommentStatus } from '@/api/cartoonReview'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listCartoonComment(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除编号为"' + Ids + '"的数据项?', '警告', {
        ...
        return deleteCartoonComment({ ids: Ids })
      }).then(() => {
        this.getList()
    ...
    changeStatues(item) {
      ...
      this.$confirm(`是否修改id:${Ids}的状态为${item.label}?`, '警告', {
        ...
        editCartoonCommentStatus({ ids: this.ids, status: item.value }).then(res => {
          this.getList()
