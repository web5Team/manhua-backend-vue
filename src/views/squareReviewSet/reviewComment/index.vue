    ...
    <template #wrapper>
      <el-card class="box-card">
        <el-form ref="formData" :inline="true" :model="formData" size="small">
          ...
          <el-form-item label="发帖id" prop="recommendId">
            <el-input
              v-model.number="formData.recommendId"
              placeholder="请输入发帖id"
              clearable
              size="small"
              @input="() => enterNumber(formData.recommendId, formData, 'recommendId')"
              @keyup.enter.native="handleQuery"
            />
          </el-form-item>
          <el-form-item label="发帖人id" prop="recommendMemberId">
            <el-input
              v-model.number="formData.recommendMemberId"
              placeholder="请输入发帖人id"
              clearable
              size="small"
              @input="() => enterNumber(formData.recommendMemberId, formData, 'recommendMemberId')"
              @keyup.enter.native="handleQuery"
            />
          </el-form-item>
          ...
          <el-form-item label="评论内容" prop="content">
            <el-input v-model="formData.content" placeholder="请输入评论内容" clearable size="small" @keyup.enter.native="handleQuery" />
          </el-form-item>
        ...
        <div style="margin-top: 15px">
          <el-button
            v-permisaction="['squareReview:comment:remove']"
            type="danger"
            size="mini"
            icon="el-icon-delete"
            ...
            >删除</el-button
          >
          <el-dropdown v-permisaction="['squareReview:comment:change']" style="margin-left: 10px" @command="changeStatues">
        ...
        >
          ...
          <el-table-column label="发帖id" sortable="custom" width="100" align="center" prop="recommendId" :show-overflow-tooltip="true" />
          <el-table-column label="发帖人id" width="100" align="center" prop="recommendMemberId" :show-overflow-tooltip="true" />
          <el-table-column label="评论人id" sortable="custom" align="center" width="100" prop="memberId" />
...
<script>
import { squareReviewCommentList, editSquareReviewCommentStatus, delSquareReviewComment } from '@/api/squareReviewApi'
export default {
  name: 'FilmComment',
  data() {
    return {
  ...
  methods: {
    ...
    async getList() {
      this.loading = true
      try {
        const { data } = await squareReviewCommentList(this.formData)
    ...
    deleteData(row) {
      ...
      this.$confirm(`是否确认删除 ${ids} 数据项?`, '警告', {
        ...
        delSquareReviewComment(ids).then((res) => {
    ...
    upStatus(row, status) {
      ...
      this.$confirm(`是否修改id:${ids}的状态为${statusName}?`, '警告', {
        ...
        editSquareReviewCommentStatus({ ids, status, memberIds: memberIds.join(',') }).then(() => {
