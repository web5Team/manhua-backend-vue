    ...
    <template #wrapper>
      <el-card class="box-card">
        ...
        >
          ...
          <el-table-column prop="menuId" label="ID" width="60px" />
            ...
            <template slot-scope="scope">
              <el-popover v-if="scope.row.sysApi.length>0" trigger="hover" placement="top">
                ...
                >
                  ...
                  >
                    <template slot-scope="scope">
                      <span v-if="scope.row.type=='SYS' && scope.row.title!=''"><el-tag type="success">{{ '['+scope.row.action +'] '+ scope.row.title }}</el-tag></span>
                      <span v-if="scope.row.type!='SYS' && scope.row.title!=''"><el-tag type="">{{ '['+scope.row.action +'] '+scope.row.title }}</el-tag></span>
              ...
              <el-row>
                ...
                <el-col v-if="form.menuType == 'M' || form.menuType == 'C'" :span="12">
                  <el-form-item prop="component">
                    <span slot="label">
                      组件路径
                      <el-tooltip content="菜单对应的具体vue页面文件路径views的下级路径/sys-api/index；目录类型：填写Layout，如何有二级目录请参照日志目录填写；" placement="top">
                        <i class="el-icon-question" />
                      </el-tooltip>
                    </span>
    ...
    getApiList() {
      ...
      listSysApi({ 'pageSize': 10000, 'type': 'BUS' }).then(response => {
        ...
        for (let i = 0; i < dataList.length; i++) {
          dataList[i].title = '[' + dataList[i].action + '] ' + dataList[i].title + '[' + dataList[i].path +  ']'
        }
        this.sysapiList = dataList
        this.loading = false
      }
      )
    },
